...
=begin
Ahora intenta crear un método que se llame anagrams_for el cual deberá recibir una palabra y un arreglo de palabras (un diccionario)
y
regresar un arreglo de las palabras que sean anagramas de esa palabra o un arreglo vació si no encuentra ninguna.
...
def anagrams_for(word,diccionario)
	result_array = []
	#CANONICAL method to each element of the array
	diccionario.select do |i| #COMPARE word with dictinary, use .select? method
		result_array << i  if canonical(i) == canonical(word)#IF the comparison is truee thenn it is put it inn the result_array
	end
	result_array
end
p anagrams_for("sopa" , ["sopa","puño","tope","paso"])
